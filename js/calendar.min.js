...
Date.prototype.getWeek = function () {
        ...
        return a.each(i, function (b, c) {
            ...
            if (a.each(b.split(">"), function (a, c) {
                    ...
                    if (j = /^(\d\d)-(\d\d)$/.exec(c))k = new Date(f, parseInt(j[2], 10) - 1, parseInt(j[1], 10)); else if (j = /^(\d\d)-(\d\d)-(\d\d\d\d)$/.exec(c))parseInt(j[3], 10) == f && (k = new Date(f, parseInt(j[2], 10) - 1, parseInt(j[1], 10))); else if (j = /^easter(([+\-])(\d+))?$/.exec(c))k = h(f, j[1] ? parseInt(j[1], 10) : 0); else if (j = /^(\d\d)([+\-])([1-5])\*([0-6])$/.exec(c)) {
                        ...
                        switch (m) {
                            case"+":
                                for (var p = new Date(f, l, -6); p.getDay() != o;)p = new Date(p.getFullYear(), p.getMonth(), p.getDate() + 1);
                                k = new Date(p.getFullYear(), p.getMonth(), p.getDate() + 7 * n);
                                break;
                            case"-":
                                for (var p = new Date(f, l + 1, 7); p.getDay() != o;)p = new Date(p.getFullYear(), p.getMonth(), p.getDate() - 1);
                                k = new Date(p.getFullYear(), p.getMonth(), p.getDate() - 7 * n)
                        }
                    }
                ...
                var j = [];
                if (g)for (var l = new Date(d.getTime()); l.getTime() <= g.getTime(); l.setDate(l.getDate() + 1))j.push(new Date(l.getTime())); else j.push(d);
    ...
    d.cache = {}, f.prototype.setOptions = function (b) {
        ...
        m.setHours(i[0]), m.setMinutes(i[1]), b.all_day = [], b.by_hour = [], b.after_time = [], b.before_time = [], a.each(b.events, function (a, d) {
            ...
            if (d.start_hour = f.getHours().toString().formatNum(2) + ":" + f.getMinutes().toString().formatNum(2), d.end_hour = g.getHours().toString().formatNum(2) + ":" + g.getMinutes().toString().formatNum(2), d.start < l.getTime() && (e(1), d.start_hour = f.getDate() + " " + c.locale["ms" + f.getMonth()] + " " + d.start_hour), d.end > m.getTime() && (e(1), d.end_hour = g.getDate() + " " + c.locale["ms" + g.getMonth()] + " " + d.end_hour), d.start < l.getTime() && d.end > m.getTime())return void b.all_day.push(d);
        ...
        var e = {
            tooltip: "",
            cal: this
        }, f = this.options.classes.months.outmonth, g = this.options.position.start.getDay();
                ...
                "iframe" == b.options.modal_type && (d = a(document.createElement("iframe")).attr({
                    ...
                    "iframe" == b.options.modal_type && (d.attr("src", f), a(".modal-body", c).html(d)), (!c.data("handled.bootstrap-calendar") || c.data("handled.bootstrap-calendar") && c.data("handled.event-id") != e.id) && c.off("show.bs.modal").off("shown.bs.modal").off("hidden.bs.modal").on("show.bs.modal", function () {
                        ...
                        switch (b.options.modal_type) {
                            ...
                            case"ajax":
                                a.ajax({
                                    url: f, dataType: "html", async: !1, success: function (a) {
                                        d.html(a)
                                    }
                                });
                                break;
